...
Recall 是一个 macOS 个人记忆捕获工具。用户可以从网页或任意本地应用中快速保存一段有价值的内容，补充一句自己的备注，随后由 AI 自动理解上下文、生成情境摘要、整理标签，并支持日后的关键词和语义检索。
...
Recall 保存的不是孤立文本，而是一次“发现”的完整情境：
...
用户使用全局快捷键，Recall 通过系统选区或剪贴板取得文本，然后保存内容和来源应用。
...
Open the Recall app and try again.
```
...
> Recall captures not only the content, but the context of why you saved it.
...
> With Recall, I select the useful part and add the context only I know.
...
> Recall combines the source, surrounding page context, and my note to create a compact memory—not just a generic summary.
...
> And because Recall is a macOS app rather than only a browser bookmark, the same workflow can capture information from local apps.

### 110–120 秒：结束

旁白：

> Recall turns scattered fragments into searchable personal context.
...
Recall 从浏览器和 macOS 应用捕获文本，结合来源上下文和用户备注，自动生成可检索的个人记忆。
